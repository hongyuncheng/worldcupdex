...
| 文件 | 触发条件 | 作用 |
| ---- | -------- | ---- |
| `.github/workflows/ci.yml` | `pull_request` → `main` | 在 Node.js 22 环境运行 `npm ci` + `npm run build` 做最小冒烟，构建失败即阻断合并 |
...
- **构建失败**：常见原因
  ...
  - `npm run build` 失败：本地复现 `npm run build`；注意 Node 版本必须为 **22**。
...
2. `actions/setup-node@v4`（Node.js 22 + npm 缓存）+ `actions/cache@v4` 缓存 `node_modules`
...
> **Node 版本升级说明**：Node 版本因 @nuxt/content 启用 native sqlite connector（`experimental.sqliteConnector: 'native'`，依赖 Node ≥ 22.5 内建 `node:sqlite`）升至 22。
